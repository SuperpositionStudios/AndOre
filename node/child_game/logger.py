...
class Logger:
	def __init__(self, region_name, log_level):
		...
		self.tick = '0,000,000'
		self.log_directory = helper_functions.path_to_this_files_directory().replace('/child_game', '') + 'logs/'
	...
	def update_tick(self, new_tick: int):
		self.tick = helper_functions.commafy(new_tick, 7)
	...
	def log(self, message, priority=0):
		...
		if priority >= self.log_level:
			with open(self.filename, 'a') as file:
				file.write(f'{helper_functions.get_date_and_time()} | Tick: {self.tick} | Priority: {priority} | {message}\n')
